...
If older version is needed, specify version in the command above, for example: qweec-0.6.6.sh  
...
The panel is free to use. It relies only on donations.  
It is created in an oldschool fashion with simple web design, following the KISS, WYSIWYG principles and "UX first" philosophy in mind.  

It was inspired by first Vesta control panel, when it's old version started to fade out.  
It is not a rewrite of Vesta, it is a different panel with it's own implementation, made from scratch, at the same time, some features, configs and template formats are intentionally made to look similar, so people, who are used to working in Vesta, could feel at home using this panel too.  

The panel is suited primarily for PHP websites, but nginx and httpd templates can be manually created and used within panel for serving other web systems (Python, NodeJS, etc)  

The project is still in the early developement and not recommended for production use yet, but already works and can be used in a developement or other non-critical environments.  
...
Docs: [https://qweec.net/doc.html](https://qweec.net/doc.html)  
CLI info: [https://qweec.net/cli.html](https://qweec.net/cli.html)  
Donation: [https://qweec.net/donate.html](https://qweec.net/donate.html)
